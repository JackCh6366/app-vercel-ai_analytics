...
export default async function handler(req: any, res: any) {
  ...
  try {
    const { transcript, targetLanguage, styleOption, provider = "gemini" } = req.body;
    ...
    if (provider === "nvidia") {
      ...
      if (!NVIDIA_API_KEY) {
        return res.status(500).json({
          error: "伺服器偵測到您尚未設定 NVIDIA_API_KEY！\n請先前往 Vercel 環境變數中，新增並設定您的 NVIDIA_API_KEY 變數，方能正常體驗 NVIDIA AI 總結功能。"
        });
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(`NVIDIA API 回傳錯誤 (${response.status}): ${errorText}`);
      }

      const data = await response.json();
      const processedResult = data.choices?.[0]?.message?.content || "NVIDIA AI 生成失敗。遺憾未回傳任何文本。";
      return res.status(200).json({ result: processedResult });
      ...
      if (!GEMINI_API_KEY) {
        return res.status(500).json({
          error: "伺服器偵測到您尚未設定 GEMINI_API_KEY！\n請先前往 Vercel 環境變數中，新增並設定您的 GEMINI_API_KEY 變數，方能正常體驗 Gemini AI 總結功能。"
        });
      }
      ...
      const processedResult = response.text || "Gemini AI 生成失敗。遺憾未回傳任何文本。";
      return res.status(200).json({ result: processedResult });
    }

  } catch (apiError: any) {
    console.error("API Serverless Function error:", apiError);
    return res.status(500).json({
      error: `AI 訊息處理失敗：${apiError?.message || "未知錯誤"}。如果您是在預覽或開發模式下，請確認您已正確配置了有效的 API 密鑰。`
    });
  }
}
